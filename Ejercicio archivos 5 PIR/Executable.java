...
public class Executable {
	 ...
	 */
	private static Employee createEmployee(Scanner sc, String dni) {
		...
		} while (tempAge < Employee.MIN_AGE || tempAge > Employee.MAX_AGE);

		double tempSalary;
		do {
			tempSalary = CustomInput.requestDouble(sc, "Sueldo: ");
		} while (tempSalary < Employee.MIN_SALARY || tempSalary > Employee.MAX_SALARY);

		return new Employee(dni, tempFName, tempLName, tempAge, tempSalary);
	}
	 ...
	 * @return Empleado creado
	 */
	private static Employee requestEmployee(Scanner sc, FileHandling fileHandler) {
		...
		do {
			...
			tempID = Employee.normalizeDni(tempID);
			isValid = Employee.isValidId(tempID);
	...
	private static void modifyEmployee(Scanner sc, FileHandling fileHandler) {
		// Pedir DNI
		var id = Employee.normalizeDni(CustomInput.requestInput(sc, "Introduzca el DNI a modificar: "));
	...
	private static void deleteEmployee(Scanner sc, FileHandling fileHandler) {
		// Pedir DNI
		var id = Employee.normalizeDni(CustomInput.requestInput(sc, "Introduzca el DNI a eliminar: "));
		// Si no es válido avisar
		if (!Employee.isValidId(id)) {
			System.err.println("DNI no válido");
			return;
	...
	private static void listInRange(Scanner sc, FileHandling fileHandler) {
		...
		do {
			...
			if (min < Employee.MIN_SALARY || min > Employee.MAX_SALARY)
				System.out.printf("Error, el salario debe estar comprendido entre %.2f y %.2f €%n", Employee.MIN_SALARY,
						Employee.MAX_SALARY);
		} while (min < Employee.MIN_SALARY || min > Employee.MAX_SALARY);
		// Pedir máximo con validación de rango
		double max = 0;
		do {
			max = CustomInput.requestDouble(sc, "Sueldo máximo: ");
			if (max < min || max > Employee.MAX_SALARY)
				System.out.printf("Error, el salario debe estar comprendido entre %.2f y %.2f €%n", min,
						Employee.MAX_SALARY);
		} while (max < min || max > Employee.MAX_SALARY);
		// Listar en el rango
